...
const TableItem = props => {
	...
	const buttonClass = 'p-2 hover:bg-purple-800 hover:text-white w-full';
	const disabledButtonClass = 'p-2 w-full opacity-50 cursor-not-allowed';
	...
	return (
		<div className={props.active === props.item._id ? active : rowClass}>
			...
			<div className='table-cell w-1/12'>
				<button type='button' className={props.online ? buttonClass : disabledButtonClass} onClick={() => props.edit()} disabled={!props.online}>
					Edit
				</button>
			</div>
		</div>
